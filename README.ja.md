...
Kagami は匿名で二つの粒度のデータを収集します。

1. **マクロ粒度**: 診断全体に対する 3 件法評価（有用 / 一部有用 / あまり有用でない）。
2. **ミクロ粒度**: 各指摘に対する同意 / 不同意の二値投票（層タグ付き）。

ミクロデータにより層別受容率を分析できます。Grammar -> Register -> Pragmatics の順で受容率が低下するなら、学習者のメタ語用意識が文法知識に比べて遅れている可能性を示し、Teachability Hierarchy と整合的な所見となります。
...
Kagamiは第二言語習得・NLP研究のため、匿名で2粒度のフィードバックを収集します。すなわち、診断全体に対する3件法評価と、文法・レジスター・語用の各層に紐づく個別指摘への同意/不同意です。これにより層別受容率を分析でき、学習者が文法—レジスター—語用の連続体でどのように診断を受け入れるか（メタ語用意識の代理指標）を捉えられますが、学習者評価をそのままLLM精度の真値とはみなしません。今後は少数の母語話者によるゴールドアノテーションを導入し、LLM-vs-Gold・Learner-vs-Gold・Learner-vs-LLMの三者比較を行う予定です。
